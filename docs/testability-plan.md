...
- [ ] **T0.2.1** Create `src/lib.rs` and move module declarations out of `main.rs` while preserving visibility and behavior.
- [ ] **T0.2.2** Update `main.rs` to consume the library crate as a thin binary entry point.
- [ ] **T0.2.3** Move CLI dispatch into a library `run` function that accepts parsed CLI arguments while leaving process setup in `main.rs`.
...
- [ ] **T1.1.1** Introduce `SessionExit` values for clean disconnect, retry, restart request, and fatal termination.
...
- [ ] **T1.3.1** Introduce a focused `TrustStore` interface.
